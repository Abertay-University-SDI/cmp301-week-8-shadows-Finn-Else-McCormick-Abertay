...
TessellationMesh::TessellationMesh(const DeviceInfo& info) : BaseMesh(info)
{
	InitBuffers();
}

// Build triangle (with texture coordinates and normals).
void TessellationMesh::InitBuffers()
{
	VertexType* vertices;
...
void TessellationMesh::SendData(D3D_PRIMITIVE_TOPOLOGY top)
{
	unsigned int stride;
	unsigned int offset;
